...
The first MVP vertical slice is implemented: a server-resolved Couple can create
a wedding, add a guest, create a private invitation, receive an account-free
...
`npm run check` formats/checks source, lints, type-checks, runs tests, and builds
both Workers. Production web builds intentionally fail if the public API origin
is absent or invalid; it is embedded at build time and never defaults to a local
server. Database query/index rationale is recorded in
